 ...
 *
 *  通过排序,我们可以有效的,把所要排序的数分割成二叉树的节点,然后仅仅需要通过与树高相同次数的归并操作,即可得到最后的结果
 *  归并排序的时间复杂度为O(nlogN)
 *
 *
 *  快速排序:
 *  快速排序,实际上是使用了一种分而治之的思想.这个思想是将大的问题,分割成解决形式相似的小的问题,然后通过解决小的问题,最后解决我们要求解的问题
 *
 *  快速排序的思想是:  首先,将一个序列尽可能的分割成左边的数小于某个标志位的数,右边的数大于某个标志位的数,然后对左边的部分以及右边的部分做相似的处理
 *
 *  1.快速排序的步骤:
 *      (1).将序列的第一个元素设置为标志位,FLAG
 *      (2).设定两个指针,分别指向这个序列的头和尾,从尾指针指向的元素开始往前找,找到一个比Flag小的数,放到头指针指向的位置中
 *      (3)从头指针指向的位置往后面找,找到一个比FLAG元素大的数,然后将这个数填充到上一步操作中空出来的数的位置
 *      (4)重复2,3,直到两个指针重合,将FLAG的值填到两个指针重合的位置
 *      (5)这个时候,序列被FLAG分为两个部分,左边部分比FLAG小,右边大.然后对两个部分做(2)(3)(4)步相同的操作,直到序列中只有一个元素
 *      算法结束
 *
 *    2.需要注意的地方.
 *    什么时候快速排序的时间复杂度最高,是多少?
 *    初始的待排序列是已经拍好序的,时间复杂度是O(n^2)
 *
 *    什么时候快速排序的时间复杂度最低,是多少?
 *    Flag的元素,刚好可以把整个序列分为左右两个部分,这两个部分的元素个数相等,或者仅仅相差1
 *
 *    k(一堆数组的下标)=2+3*(i-2)+j-i+1=2i+j-3
 * */